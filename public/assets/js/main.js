...
// hero-img 
  ...
  $(".vs-carousel").each(function () {
    ...
    // Skip if element doesn't exist in DOM anymore
    if (!asSlide.length) return;
    
    // Vérifier si l'élément est visible dans le DOM
    if (!asSlide.is(':visible')) return;
    ...
    $("[data-slick-next]").each(function () {
      $(this).on("click", function (e) {
        ...
        if (target.length) target.slick("slickNext");
      });
    });
    ...
    $("[data-slick-prev]").each(function () {
      $(this).on("click", function (e) {
        ...
        if (target.length) target.slick("slickPrev");
      });
    });

    // Check for arrow wrapper
    if (d("arrows") == true) {
      var container = asSlide.closest(".container");
      if (!asSlide.closest(".arrow-wrap").length && container && container.parent().length) {
        container.parent().addClass("arrow-wrap");
      }
    }

    try {
      // Vérifier si slick est déjà initialisé sur cet élément
      if (asSlide.hasClass('slick-initialized')) {
